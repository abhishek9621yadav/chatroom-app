...
export default function ChatPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (roomId && data.chatrooms) {
          const targetChatroom = data.chatrooms.find((room: Chatroom) => room._id === roomId)
          if (targetChatroom) {
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
      {/* Sidebar */}
      <div
        className={`${isMobile ? (showSidebar ? "w-full" : "hidden") : "w-80 border-r border-gray-200 dark:border-gray-700"} bg-white dark:bg-gray-800 flex flex-col`}
      >
        <ChatSidebar
          chatrooms={chatrooms}
      ...
      {/* Chat Window */}
      <div className={`${isMobile ? (showSidebar ? "hidden" : "w-full") : "flex-1"} flex flex-col`}>
        {selectedChatroom ? (
          <ChatWindow chatroom={selectedChatroom} user={user} onBack={handleBackToSidebar} isMobile={isMobile} />
        ) : (
          <div className="flex-1 flex items-center justify-center bg-gray-50 dark:bg-gray-900">
            <div className="text-center">
                ...
                <svg viewBox="0 0 303 172" className="w-full h-full">
                  <defs>
                    <linearGradient id="a" x1="50%" x2="50%" y1="100%" y2="0%">
                      <stop offset="0%" stopColor="#1fa2f3" stopOpacity=".7"></stop>
                      <stop offset="100%" stopColor="#12d886" stopOpacity=".7"></stop>
                    </linearGradient>
                  </defs>
                  <path
                    fill="url(#a)"
              ...
              <h2 className="text-2xl font-light text-gray-500 dark:text-gray-400 mb-2">RoomTalk</h2>
              <p className="text-gray-400 dark:text-gray-500 max-w-md">
                Send and receive messages without keeping your phone online. Use RoomTalk on up to 4 linked devices and
